...
func (c *RootCmd) Usage() {
	err := executeUsage(os.Stderr, "io_types_usage.txt", UsageDataRootCmd{c, false})
	if err != nil {
...
func (c *RootCmd) UsageRecursive() {
	err := executeUsage(os.Stderr, "io_types_usage.txt", UsageDataRootCmd{c, true})
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error generating usage: %s\n", err)
	}
}
...
func NewRoot(name, version, commit, date string) (*RootCmd, error) {
	...
	c.CommandAction = func(c *RootCmd) error {
		...
		if err != nil {
			...
			return fmt.Errorf("io_types failed: %w", err)
		}
		return nil
	}

	{
		subCmd := NewLazyCommand(func() Cmd { return c.NewCopyFile() })
		c.Commands["copy-file"] = subCmd

	}

	{
		subCmd := NewLazyCommand(func() Cmd { return c.NewInspectFile() })
		c.Commands["inspect-file"] = subCmd
