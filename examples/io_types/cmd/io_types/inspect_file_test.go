...
func TestInspectFile_Execute(t *testing.T) {

	parent := &RootCmd{
		FlagSet:  flag.NewFlagSet("root", flag.ContinueOnError),
		Commands: make(map[string]func() Cmd),
	}
	cmd := parent.NewInspectFile()

	called := false
	cmd.CommandAction = func(c *InspectFile) error {
		called = true
		return nil
	}
...
func TestInspectFile_ExecuteHelpAndUnknownFlags(t *testing.T) {
