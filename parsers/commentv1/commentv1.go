...
func collectSubCommands(cmd *model.Command, name string, sct *SubCommandTree, parent *model.SubCommand, allocator *parsers.NameAllocator) []*model.SubCommand {
	...
	if sct.SubCommand != nil {
		...
		if name == "" {
			...
			allocatedStructName := allocator.Allocate(allocateName)
			syntheticCmd := &model.SubCommand{
				Command:                cmd,
				Parent:                 parent,
				SubCommandName:         name,
				SubCommandStructName:   allocatedStructName,
				ConstructorMethodName:  allocator.Allocate("New" + allocatedStructName),
...
var (
	...
	reCliParser       = regexp.MustCompile(`(?i)^CLI-Parser:\s*([A-Za-z0-9][A-Za-z0-9-]*)\s*package commentv1
...
var reImplicitParam = regexp.MustCompile(`^([\w]+):\s*(.*)package commentv1
