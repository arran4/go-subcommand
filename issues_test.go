...
func runGenerateInMemory(t *testing.T, inputFS fstest.MapFS) *MockWriter {
	...
	if err := GenerateWithFS(inputFS, writer, ".", "", "commentv1", false); err != nil {
...
`
	...
	err := GenerateWithFS(fs, writer, ".", "", "commentv1", false)
...
func MyCmd() {}
`
	// 1. Existing manual file without header -> Should Fail
	t.Run("ManualFile_Fail", func(t *testing.T) {
		fs := setupProject(t, src)
		// Simulate existing manual file in target location
		fs["cmd/app/mycmd.go"] = &fstest.MapFile{Data: []byte("package main\n// manual content")}
		writer := NewMockWriter()

		err := GenerateWithFS(fs, writer, ".", "", "commentv1", false)
		if err == nil {
			t.Error("Expected error when overwriting manual file without force, got nil")
		} else if !strings.Contains(err.Error(), "missing header") {
			t.Errorf("Expected 'missing header' error, got: %v", err)
		}
	})

	// 2. Existing manual file WITH force -> Should Succeed
	t.Run("ManualFile_Force_Succeed", func(t *testing.T) {
		fs := setupProject(t, src)
		fs["cmd/app/mycmd.go"] = &fstest.MapFile{Data: []byte("package main\n// manual content")}
		writer := NewMockWriter()

		err := GenerateWithFS(fs, writer, ".", "", "commentv1", true)
		if err != nil {
			t.Errorf("Expected success with force, got error: %v", err)
		}
	})

	// 3. Existing generated file (with header) -> Should Succeed
	t.Run("GeneratedFile_Succeed", func(t *testing.T) {
		fs := setupProject(t, src)
		// Simulate existing generated file
		fs["cmd/app/mycmd.go"] = &fstest.MapFile{Data: []byte("// Code generated by github.com/arran4/go-subcommand/cmd/gosubc. DO NOT EDIT.\npackage main")}
		writer := NewMockWriter()

		err := GenerateWithFS(fs, writer, ".", "", "commentv1", false)
		if err != nil {
			t.Errorf("Expected success for generated file, got error: %v", err)
		}
	})

	// 4. Existing generated file (with alternative header) -> Should Succeed
	t.Run("GeneratedFile_AltHeader_Succeed", func(t *testing.T) {
		fs := setupProject(t, src)
		// Simulate existing generated file with "Generated by" instead of "Code generated by"
		fs["cmd/app/mycmd.go"] = &fstest.MapFile{Data: []byte("// Generated by github.com/arran4/go-subcommand/cmd/gosubc\npackage main")}
		writer := NewMockWriter()

		err := GenerateWithFS(fs, writer, ".", "", "commentv1", false)
		if err != nil {
			t.Errorf("Expected success for generated file with alt header, got error: %v", err)
		}
	})
}
...
func MyCmd() {}
`
	// 1. Extraneous file in cmd/ -> Should Fail
	t.Run("ExtraneousFile_Fail", func(t *testing.T) {
		fs := setupProject(t, src)
		// Simulate extraneous file
		fs["cmd/extraneous.go"] = &fstest.MapFile{Data: []byte("package main")}
		writer := NewMockWriter()

		err := GenerateWithFS(fs, writer, ".", "", "commentv1", false)
		if err == nil {
			t.Error("Expected error for extraneous file without force, got nil")
		} else if !strings.Contains(err.Error(), "extraneous file found") {
			t.Errorf("Expected 'extraneous file found' error, got: %v", err)
		}
	})

	// 2. Extraneous file in cmd/ WITH force -> Should Succeed
	t.Run("ExtraneousFile_Force_Succeed", func(t *testing.T) {
		fs := setupProject(t, src)
		fs["cmd/extraneous.go"] = &fstest.MapFile{Data: []byte("package main")}
		writer := NewMockWriter()
	...
	// 3. Extraneous file OUTSIDE cmd/ -> Should Succeed (Ignored)
	t.Run("ExternalFile_Succeed", func(t *testing.T) {
		fs := setupProject(t, src)
		// File outside cmd/
		fs["other/extraneous.go"] = &fstest.MapFile{Data: []byte("package main")}
		writer := NewMockWriter()

		err := GenerateWithFS(fs, writer, ".", "", "commentv1", false)
		if err != nil {
			t.Errorf("Expected success for external file, got error: %v", err)
		}
	})
}
