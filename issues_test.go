...
func runGenerateInMemory(t *testing.T, inputFS fstest.MapFS) *CollectingFileWriter {
	...
	if err := GenerateWithFS(inputFS, writer, ".", "", "commentv1", nil, false, false, nil); err != nil {
...
`
	...
	err := GenerateWithFS(fs, writer, ".", "", "commentv1", nil, false, false, nil)
...
`
	...
	writer := NewCollectingFileWriter()
	// Generate code
	if err := GenerateWithFS(fs, writer, ".", "", "commentv1", nil, false, false, nil); err != nil {
		t.Fatalf("Generate failed: %v", err)
	}
...
`
	...
	err = GenerateWithFS(fs, writer, ".", "", "commentv1", nil, true, false, nil)
	if err != nil {
