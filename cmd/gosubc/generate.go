...
func (c *Generate) Execute(args []string) (err error) {
	...
	for i := 0; i < len(args); i++ {
		...
		if strings.HasPrefix(arg, "--") {
			...
			case "cliParser", "cli-parser", "flag":
				if !hasValue {
					if i+1 < len(args) {
						value = args[i+1]
						i++
			...
			case "replaceTemplates", "replace-template":
				if !hasValue {
					if i+1 < len(args) {
						value = args[i+1]
						i++
...
func (c *RootCmd) NewGenerate() *Generate {
	...
	set.StringVar(&v.cliParser, "cli-parser", "gnu", "Runtime CLI parser backend (gnu, go)")
	set.StringVar(&v.cliParser, "flag", "gnu", "Runtime CLI parser backend (gnu, go)")
	...
	set.BoolVar(&v.force, "force", false, "Force overwrite of files not generated by gosubc")
