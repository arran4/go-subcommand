// Code generated by github.com/arran4/go-subcommand/cmd/gosubc version (devel). DO NOT EDIT.

package main

import (
	"flag"
	"testing"
)

func TestTemplateExport_Execute(t *testing.T) {

	parent := &Template{}
	parent.RootCmd = &RootCmd{
		FlagSet:  flag.NewFlagSet("root", flag.ContinueOnError),
		Commands: make(map[string]func() Cmd),
	}
	cmd := parent.NewTemplateExport()

	called := false
	cmd.CommandAction = func(c *TemplateExport) error {
		called = true
		return nil
	}

	args := []string{}
	args = append(args, "--output")
	args = append(args, "test")
	args = append(args, "--as-txtar")

	err := cmd.Execute(args)
	if err != nil {
		t.Errorf("Unexpected error: %v", err)
	}

	if !called {
		t.Error("CommandAction was not called")
	}

	if cmd.output != "test" {
		t.Errorf("Expected output to be 'test', got '%v'", cmd.output)
	}
	if cmd.asTxtar != true {
		t.Errorf("Expected asTxtar to be true, got '%v'", cmd.asTxtar)
	}
}

func TestTemplateExport_ExecuteHelpAndUnknownFlags(t *testing.T) {

	parent := &Template{}
	parent.RootCmd = &RootCmd{
		FlagSet:  flag.NewFlagSet("root", flag.ContinueOnError),
		Commands: make(map[string]func() Cmd),
	}
	cmd := parent.NewTemplateExport()

	if err := cmd.Execute([]string{"--help"}); err != nil {
		t.Errorf("--help returned an error: %v", err)
	}
	if err := cmd.Execute([]string{"-h"}); err != nil {
		t.Errorf("-h returned an error: %v", err)
	}
	if err := cmd.Execute([]string{"--not-a-real-flag"}); err == nil {
		t.Error("expected an error for an unknown long flag")
	}
	if err := cmd.Execute([]string{"-?"}); err == nil {
		t.Error("expected an error for an unknown short flag")
	}
}
