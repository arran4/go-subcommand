...
func NewRoot(name, version, commit, date string) (*RootCmd, error) {
	...
	{
		sub := c.NewFormat()
		c.Commands["format"] = sub
	}
	{
		sub := c.NewFormatSourceComments()
		c.Commands["format-source-comments"] = sub
	}
	{
		sub := c.NewGenerate()
		c.Commands["generate"] = sub
	}
	{
		sub := c.NewGoreleaser()
		c.Commands["goreleaser"] = sub
	}
	{
		sub := c.NewList()
		c.Commands["list"] = sub
	...
	{
		sub := c.NewSyntax()
		c.Commands["syntax"] = sub
	}
	{
		sub := c.NewValidate()
		c.Commands["validate"] = sub
	}
