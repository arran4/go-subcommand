...
* **Test Filename Matching:** Test files must match the source filename without adding any suffixes (e.g., tests for `foo.go` should be in `foo_test.go`, not `foo_extra_test.go` or `foo_tree_test.go`). This ensures clear organization and predictability.
...
* **Parameter Configuration Structures:** Parameter parsing now uses structured fields: `model.ParserConfig` and `model.GeneratorConfig`, which include `Type` constants and a nested pointer `Func` to a `FuncRef` representing `ImportPath`, `PackagePath`, `CommandPackageName`, and `FunctionName`. This replaces the previous string-only implementation. Templates must use `{{if .Parser.Func}}{{if .Parser.Func.FunctionName}}` instead of string lookups.
* **Testing Extensibility:** Txtar (.txtar) integration tests should be updated to strictly adhere to the expected input layout (`input/`) and expected output layout (`expected/`) to fit within the `e2e_test.go` framework. Ensure new features are tested thoroughly via this integration.
* **Template Updates:** When modifying templates (such as `cmd.go.gotmpl`, `root.go.gotmpl`, or `common.gotmpl`), use proper nested struct field dereferencing as noted above. Ensure `gotmpl` spaces don't emit trailing whitespace where `usage.txt.gotmpl` verifies it.
* **Keep Documentation Complete:** Always ensure that documentation is complete and up to date, including the `README.md`, man pages, and any other relevant documentation.
