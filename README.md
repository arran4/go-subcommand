...
### Positional Argument Algorithms

By default, optional positional arguments are matched greedily by index. If you have complex positional arguments with custom parsers, this may cause arguments to fail parsing because they were incorrectly greedily consumed.

You can switch the matching algorithm for optional positional arguments to a `best-fit` approach by including `[positional-algorithm: best-fit]` or `(positional-algorithm: best-fit)` in your command documentation. This algorithm will attempt to map remaining arguments to positionals sequentially, and will skip an optional argument and continue to the next one if it fails parsing (e.g. failing a custom parser or type conversion).

```go
// MyCmd is a subcommand `app complex`
// [positional-algorithm: best-fit]
func MyCmd(a string, b int, c string) { ... }
```

### GNU-Style `--` Termination (Parser Architecture)
...
*   **Termination Detection:** The first standalone `--` token encountered halts all flag parsing and subcommand recognition for the current command.
*   **Positional Passthrough:** All tokens following `--` (including subsequent `--` tokens, unknown flags, or subcommand names) are treated strictly as positional arguments and passed through untouched.
*   **Command Scope:** The termination is contextual to the command level where it is encountered; an `app -- subcommand` passes `subcommand` as an argument to `app`, while `app subcommand -- child` passes `child` as an argument to `subcommand`.
